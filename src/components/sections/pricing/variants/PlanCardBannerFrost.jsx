...
/**
 * V3 — Banner + frost title. Banner full-bleed and taller than V2
 * (`aspect-[4/3]` vs `16/9`), with the plan name floating at the bottom
 * of the image inside a glassmorphism pill. The image and the title
 * merge into a poster-like header.
 *
 * Intentional divergences from production PlanCard (same as V1, V2):
 *   - No `highlighted` / `delay` / `className` props.
 *   - `animate` uses a fixed `y: 0`.
 *   - All `track()` calls carry `source: 'lab_v3_banner_frost'`.
 *
 * Layout deltas vs V2 Banner:
 *   - Image taller: `aspect-[4/3]` instead of `aspect-[16/9]`.
 *   - `<h3>` is positioned `absolute bottom-4 left-4` INSIDE the image
 *     wrapper, with a glassmorphism pill (backdrop-blur, semi-transparent
 *     bg, border-white/20).
 *   - No `<h3>` in the padded zone below the banner — the title lives
 *     on the image.
 *   - When a plan has NO image, V3 falls back to a normal `<h3>` in the
 *     padded zone (matches V1/V2 hierarchy).
 *
 * Like V2, V3 uses `focus-visible:outline-*` instead of `ring-*` for
 * the stretched link, because the article's `overflow-hidden` (needed
 * for the banner to clip on the rounded corners) would otherwise clip
 * a box-shadow-painted ring.
 */
